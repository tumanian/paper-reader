...
> Branch: `modularization`. Last updated: Phase 3 chat DONE (js/chat.js extracted, 119 green). Next: Step 12 onboarding.js (main.js = entry/nav only).
>
...
- Phase 3:
  ...
  - **NEXT → Step 12:** extract `js/onboarding.js` (runOnboardingDemo, showOnboardingCitationDemo, loadOnboardingData, sanitizeOnboarding, maybeApplyOnboardingCuration, applyOnboardingItems, cancelOnboardingPlacement, getFeaturedPaper, openFeaturedExample, openOnboardingPaper, locateTextRange, rectsForRange, maybeShowOnboardingHint); onboarding imports chat (openChat/sendMessage/paintHighlight/renderList) directly + wires setChatHooks({runOnboardingDemo}). Leave main.js as entry/wiring/nav only (imports + hook wiring + boot + new-btn/backToUpload). Run suite + ESM guard, commit.
