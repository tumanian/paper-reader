...
> Branch: `modularization`. Last updated: Phase 3 figure DONE (js/figure.js extracted, 119 green). Next: Step 11 chat.js (+highlights+ratings), then Step 12 onboarding.js (main.js = entry only).
>
...
- Phase 3:
  ...
  - **NEXT → Step 11:** extract `js/chat.js` (+highlights+ratings): paintHighlight, renderList, deleteDiscussion, openChat, scrollHighlightIntoView, showList, ratings (openRatingsDB, ratingsTx, get/put/delete/getAllRatingRecords, ratingIdFor, buildRatingRecord, renderRatingControl, exportRatings), rebuildChat, addMsg, buildDocContext, findNearbyContext, buildPaperBlock, sendMessage, callClaude + msg-input listener. Many modules currently hook these (pdf/web-loader/selection/figure paintHighlight+renderList, selection openChat+sendMessage, web-loader renderList, citation-resolve findNearbyContext) — after chat extraction, replace those hooks with direct imports where clean. Run suite + ESM guard, commit.
  - **THEN → Step 12:** extract `js/onboarding.js` (FEATURE_CTA, runOnboardingDemo, showOnboardingCitationDemo, loadOnboardingData, maybeApplyOnboardingCuration, cancelOnboardingPlacement, getFeaturedPaper, openFeaturedExample, curation placement); leave main.js as the entry/wiring module only (imports + hook wiring + boot). Run suite + ESM guard, commit.
