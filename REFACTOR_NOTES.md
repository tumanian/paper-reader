...
> Branch: `modularization`. Last updated: Phase 3 Step 2 DONE (js/state.js extracted, 119 green). Next: Step 3 extract js/persistence.js.
>
...
- Phase 3:
  ...
  - **NEXT → Step 3:** extract `js/persistence.js` (the localStorage/IndexedDB/Supabase save-load glue around PaperStore: loadStore, persistCurrentDoc + its empty-overwrite guard, migrateDoc/setDefault, docIdFor, loadDocSummary?, read-later helpers as appropriate). Keep the persistCurrentDoc guard intact. Run suite + ESM guard, commit.
