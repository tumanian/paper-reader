...
> Branch: `modularization`. Last updated: Phase 3 web-loader DONE (js/web-loader.js extracted, 119 green). REORDERED: did citation-parse before web-loader. Next: Step 8 citation-resolve.js (+log).
>
...
- Phase 3:
  ...
  - **NEXT → Step 8:** extract `js/citation-resolve.js` (+log): logCitation, citation-log store (loadCitationLogStore, persistCitationLog, citeLogKey, CITE_LOG_* consts), resolveCitationUrl, lookupCrossrefBibliography, ensureCitationFormat, callCitationMatch, sampleCitationContext, fetchArxivAbstract, finishCitationNavigation, citation preview pipeline. Imports citation-parse + web-loader (arxivIdFromUrl). Run suite + ESM guard, commit.
