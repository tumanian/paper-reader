...
> Branch: `modularization`. Last updated: Phase 3 Step 5 DONE (js/pdf.js extracted, 119 green). Next: Step 6 extract js/web-loader.js (+references).
>
...
- Phase 3:
  ...
  - **NEXT → Step 6:** extract `js/web-loader.js` (+references/bibliography): `loadWebPage`, `smartRewrite`, arXiv helpers, `fetchViaProxy`/`fetchPdfViaProxy`, `renderWebArticle`, `restoreWebHighlights`, `startApp`, `setStatus`, `showViewer`, `arxivIdFromUrl`, `buildPaperReferences` + bibliography indexing. Run suite + ESM guard, commit.
