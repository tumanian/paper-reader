...
> Branch: `modularization`. Last updated: Phase 3 citation-parse DONE (js/citation-parse.js extracted, 119 green). REORDERED: did citation-parse (was step 7) before web-loader (step 6) because web-loader/references depends on the pure citation core, not vice versa — extracting the core first lets web-loader import it directly instead of via hooks. Next: web-loader.js (+references).
>
...
- Phase 3:
  ...
  - **NEXT → Step 6:** extract `js/web-loader.js` (+references/bibliography): `loadWebPage`, `smartRewrite`, arXiv helpers, `fetchViaProxy`/`fetchPdfViaProxy`, `renderWebArticle`, `restoreWebHighlights`, `startApp`, `setStatus`, `showViewer`, `arxivIdFromUrl`, `buildPaperReferences` + bibliography indexing (`indexBibliographyFromDoc`, `indexWebBibliography`, `addBibliographyItem`, `extractReferencesSectionFromDoc`, `expandSelectionText`, `parseReferencesFromSection`, `parseAuthorYearReferenceLines`, `dumpBibliography`). Can now import citation-parse directly. Run suite + ESM guard, commit.
