...
> Branch: `modularization`. Last updated: Phase 3 selection DONE (js/selection.js extracted, 119 green). Next: Step 10 figure.js.
>
...
- Phase 3:
  ...
  - **NEXT → Step 10:** extract `js/figure.js`: FIGURE CAPTURE section (figureToast, isFigureCaptureAvailable, armFigureCapture, disarmFigureCapture, onCaptureMouseDown/Move/Up, positionCaptureRect, capturePdfRegion, pdfCaptionForRect, captureWebRegion, loadImage, cropDrawableToDataUrl, rasterizeElementCrop, fetchImageViaProxy, captureDomRegionFallback, webCaptionFor, startFigureDiscussion, ensureFigureImage, renderChatFigure + capture keydown listener). Hooks into chat (openChat, sendMessage, addDiscussion via state). Run suite + ESM guard, commit.
